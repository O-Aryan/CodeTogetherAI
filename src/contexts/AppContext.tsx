...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    socket.on('codeUpdate', (newCode: string) => {
      // Always update code, even if currentRoom is not set yet
      if (state.currentRoom) {
        dispatch({ type: 'UPDATE_CODE', payload: { code: newCode, language: state.currentRoom.language } });
      } else {
        // If currentRoom is not set, set it with default language
  ...
  const actions = {
    ...
    updateCode: (code: string, language: string) => {
      ...
      if (state.currentRoom) {
        socketRef.current.emit('codeChange', { roomId: state.currentRoom.id, newCode: code });
      }
    },
