        ...
        <div className="grid grid-cols-2 gap-2">
          {quickPrompts.map((prompt, index) => (
            <button
              ...
              className="flex items-center space-x-2 p-2 rounded-lg bg-white dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors text-left border border-gray-200 dark:border-gray-600 shadow-soft"
            >
              <span className={prompt.color}>{prompt.icon}</span>
              <span className="text-xs text-gray-700 dark:text-gray-300">{prompt.text}</span>
            </button>
          ))}
        </div>
      </div>
